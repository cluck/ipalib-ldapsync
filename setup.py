...
## Copyright (c) 2016, Claudio Luck (Zurich, Switzerland)
##
...
PLUGIN = 'ldapsync'
PACKAGES = ['ipalib_ldapsync']

with codecs.open(os.path.join('.', 'ipalib_ldapsync', 'version.py')) as init:
    METADATA = dict(re.findall("__([A-Za-z][A-Za-z0-9_]+)__\s*=\s*'([^']+)'", init.read().decode('utf-8')))

if os.path.exists("README.md"):
    long_description = codecs.open("README.md", "r", "utf-8").read()
else:
    long_description = "See https://github.com/cluck/ipalib-ldapsync"
...
class post_install(install_data):
	...
	shutil.copy(
            os.path.join(self.install_dir, 'ipalib/plugins/ldapsync.py'),
            os.path.join(IPALIB_DIR, 'plugins/ldapsync.py'),
        )

extra = {}

setup(
    name = 'ipalib-ldapsync',
    packages=PACKAGES,
    version=METADATA['version'],
    description = 'RFID door control plugin for FreeIPA 4',
    long_description = long_description,
    url = 'https://github.com/cluck/ipalib-ldapsync',
    ...
    entry_points={
        'console_scripts': [
            'ldapsync = ipalib_ldapsync.commands:ldapsync',
        ],
    },
    zip_safe=False,
    classifiers=[
